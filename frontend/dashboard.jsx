                        ...
                        return (
                          <div key={category.id} className="relative">
                            <button
                              ...
                              className={`flex items-center gap-3 px-4 py-2 rounded-full border transition-colors ${expandedCategory === category.id
                      ...
                      {[
                        ...
                        return (
                          <button
                            ...
                            className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-colors ${canvasTab === tab.id
                                ? 'bg-gray-200'
                                      ...
                                      {spreadsheetData[0].map((_, colIdx) => (
                                        <td key={colIdx} className="border border-gray-300 px-0 py-0 text-gray-600">
                                          <input
                                            ...
                                            className={`w-full px-3 py-2 bg-transparent border-none outline-none focus:bg-blue-50 focus:ring-2 focus:ring-blue-400 focus:ring-inset ${isHighlighted ? 'bg-amber-100' : ''
                                              }`}
                                          />
                                        </td>
                                      ))}
                            ...
                            <div className="flex-1 overflow-auto p-3 space-y-2">
                              {reviewIssues.map((issue, idx) => (
                                <div
                                  ...
                                  className={`p-3 rounded-2xl transition-colors cursor-pointer ${highlightedRow === issue.row
                                      ? 'bg-amber-100 ring-2 ring-amber-400'
                                      : 'bg-gray-50 hover:bg-gray-100'
                                    } ${issue.row === null ? 'cursor-default' : ''}`}
                                >
                                  <div className="flex items-start gap-3">
                                    <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${issue.type === 'warning'
                                        ? 'bg-amber-100'
                                        : issue.type === 'success'
                                          ? 'bg-green-100'
                                          : 'bg-blue-100'
            ...
            {consoleLogs.length === 0 ? (
              ...
              consoleLogs.map((log, idx) => (
                <div key={idx} className="flex gap-2 py-0.5 hover:bg-gray-800 px-2 rounded">
                  ...
                  <span className={`flex-shrink-0 ${log.type === 'error' ? 'text-red-400' :
